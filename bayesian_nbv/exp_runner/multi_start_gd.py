...
class MultiStartGradientDescentRunner(BaseRunner):
    ...
    def expected_improvement(self, f_samples: torch.Tensor, x_data: torch.Tensor, v_data: torch.Tensor, step_ckpt: dict[str, Any]) -> tuple[torch.Tensor, torch.Tensor, dict[str, Any]]:
        ...
        if True:
            for it in (pbar := trange(self.config.camera.num_iterations, desc="Multi-start gradient descent", leave=False, disable=not self.verbose)):
                ...
                def closure():
                    ...
                    t_step = t_params
                    if torch.isnan(R_step).any() or torch.isnan(t_step).any():
                        # print(f"[WARNING] NaN values in R_step or t_step, printing history...")
                        # for i, (R_i, t_i) in enumerate(zip(R_history, t_history)):
                        #     print(f"R_{i:03d}: {R_i}")
                        #     print(f"t_{i:03d}: {t_i}")
        ...
        with torch.no_grad():
            ...
            expected_acquisitions = []
            R_final = rotation_6d_to_matrix(R_params).detach()
            t_final = t_params.detach()
