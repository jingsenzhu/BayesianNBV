...
class IterativeSphereGradientDescentRunner(BaseRunner):
    def __init__(self, config: edict, verbose: bool = True):
        super().__init__(config, verbose)
        self.multi_start_batch_size = self.config.camera.multi_start_batch_size
        assert self.multi_start_batch_size > 0, "multi_start_batch_size must be positive integer"
    ...
    def expected_improvement(self, f_samples: torch.Tensor, x_data: torch.Tensor, v_data: torch.Tensor, step_ckpt: dict[str, Any]) -> tuple[torch.Tensor, torch.Tensor, dict[str, Any]]:
        ...
        eye_starts = fibonacci_sphere(self.config.camera.num_starts, radius=self.config.camera.radius, device=self.device, random_oriented=True)
        ...
        for bi in range(0, eye_starts.shape[0], self.multi_start_batch_size):
            start = bi
            end = min(start + self.multi_start_batch_size, eye_starts.shape[0])
            eye_starts_batch = eye_starts[start:end]

            eye_params = nn.Parameter(eye_starts_batch)
            ...
            for it in (pbar := trange(self.config.camera.num_iterations, desc="Multi-start gradient descent", leave=False, disable=not self.verbose)):
                ...
                def closure():
                    ...
                    expected_acquisitions_batch = torch.stack(expected_acquisitions_batch, dim=0) # (S,B)
                    expected_acquisitions_batch = expected_acquisitions_batch.mean(dim=0) # (B,)
                ...
                if self.verbose:
                    with torch.no_grad():
                        ...
                        postfix_str = f"Batch {bi:03d}, Mean: {mean_acq:.4f}, Min: {min_acq:.4f}, Max: {max_acq:.4f}"
                        pbar.set_postfix_str(postfix_str)
        
            with torch.no_grad():
                R_history_batch = np.array(R_history_batch)
                t_history_batch = np.array(t_history_batch)
                ...
                eyes_batch_final = eye_params / (eye_params.norm(dim=-1, keepdim=True) + 1e-8) * self.config.camera.radius
                R_final_batch, t_final_batch = cameras_looking_at_origin(eyes_batch_final)
                R_final_batches.append(R_final_batch)
                t_final_batches.append(t_final_batch)
                del eye_params, eyes_batch_final
                gc.collect()
        ...
        with torch.no_grad():
            R_final = torch.cat(R_final_batches, dim=0) # (C,3,3)
            t_final = torch.cat(t_final_batches, dim=0) # (C,3)
