...
def grid_3d_points(density, radius=1.0):
    """
    Mesh corresponding to [-0.5, 0.5]^3 in R^3
    """

    grid = np.mgrid[(slice(-0.5, 0.5, density * 1j),) * 3]
    v = np.stack(grid, axis=-1).reshape(-1, 3) * radius

    x1 = x2 = x3 = np.mgrid[-0.5:0.5:density * 1j] * radius
